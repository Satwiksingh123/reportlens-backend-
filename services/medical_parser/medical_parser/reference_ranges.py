...
REFERENCE_TABLE: list[BiomarkerRef] = [
    ...
    # "S.G.P.T."/"S.G.O.T." (a dot after every letter) are real report wording distinct from
    # bare "SGPT"/"SGOT" - our tokenizer treats each dot-separated letter as its own token,
    # so the dotted form needs its own alias (built from the same letters) to match; the
    # existing bare "sgpt"/"sgot" alias already covers the no-dots form.
    BiomarkerRef("LFT", "SGPT (ALT)", "U/L", 7.0, 56.0, ("alt", "sgpt", "s.g.p.t.", "alanine aminotransferase")),
    BiomarkerRef("LFT", "SGOT (AST)", "U/L", 5.0, 40.0, ("ast", "sgot", "s.g.o.t.", "aspartate aminotransferase")),
    ...
    # "Globumin" is a genuine typo baked into one real vendor's report template (confirmed
    # via the PDF's own embedded text, not an OCR misread) - accommodated since every report
    # generated by that same software carries the same typo.
    BiomarkerRef("LFT", "Globulin", "g/dL", 2.0, 3.5, ("globumin",)),
    BiomarkerRef("LFT", "A/G Ratio", None, 1.0, 2.1, ("albumin globulin ratio",)),
    BiomarkerRef("LFT", "GGT", "U/L", 8.0, 61.0, ("gamma gt", "g.g.t.p", "gamma glutamyl transferase")),
